...
class CrateScatter(gym.core.Wrapper):
    """
    Novelty wrapper to add a crate to the map.
    When broken, the crate scatters items around the map
    """

    def __init__(self, env, difficulty, contents=None):
        super().__init__(env)

        self.item_to_add = "crate"
        self.env.items.add(self.item_to_add)
        self.env.items_id.setdefault(self.item_to_add, len(self.items_id))
        ...
        self.contents = contents if contents else {'stick': 4, 'plank': 2, 'rubber': 1}

        for obj, _ in self.contents.items():
            if obj not in self.env.items:
                self.env.items.add(obj)
                self.env.items_id.setdefault(obj, len(self.items_id))

        if difficulty == 'easy':
            self.item_percent_range = (99, 100)
        elif difficulty == 'medium':
            self.item_percent_range = (40, 70)
        else:
            self.item_percent_range = (10, 30)

        crate_size = sum(self.contents.values())
        item_percent = np.random.randint(low=self.item_percent_range[0], high=self.item_percent_range[1], size=1)[0]
        num_items = int(np.ceil(crate_size * (item_percent / 100)))

        self.contents = [[k]*v for k,v in self.contents.items()]
        self.contents = [item for sublist in self.contents for item in sublist]
        self.contents = np.random.choice(self.contents, num_items, replace=False)
    ...
    def reset(self):
        ...
        # add crate to one random location in the map
    ...
    def step(self, action_id):
        ...
        if self.env.inventory_items_quantity['crate'] < 1:
            if self.block_in_front_str == 'crate' and action_id == 0: # break
                for obj in self.contents:
                    result = np.array(np.where(self.env.map == 0))
                    # check if there's room for new item
                    num_items = min(len(result[0]), 1)
                    if num_items > 0:
                        self.env.add_item_to_map(obj, 1)
...
class ScatterInventory(gym.core.Wrapper):
    """
    Novelty wrapper to add a crate to the map.
    When broken, the crate scatters items from the agent's inventory around the map
    """
    ...
    def step(self, action_id):
        ...
        if self.env.inventory_items_quantity['crate'] < 1:
            if self.block_in_front_str == 'crate' and action_id == 0: # break
                ...
                self.contents = copy.deepcopy(self.env.inventory_items_quantity)
                ...
                for obj in self.contents:
                    ...
                    num_items = min(len(result[0]), 1)
                    if num_items > 0:
                        self.env.inventory_items_quantity[obj] -= 1
                        self.env.add_item_to_map(obj, 1)
...
class KeyWall(gym.core.Wrapper) :

    def __init__(self, env, difficulty='hard') :
        super().__init__(env)

        self.wall_name = 'key_wall'
        self.env.items.add(self.wall_name)
        self.env.items_id.setdefault(self.wall_name, len(self.items_id))
        self.env.unbreakable_items.add(self.wall_name)

        self.key_name = 'key'
        self.env.add_new_items({self.key_name: 1})
        self.env.entities.add(self.key_name)
        self.env.select_actions_id.update({'Select_' + self.key_name: len(self.env.actions_id)})
        self.env.actions_id.update(self.env.select_actions_id)


    def reset(self) :
        obs = self.env.reset()
        self.env.block_items('crafting_table', self.wall_name)
    ...
    def step(self, action_id) :
        ...
        r, c = self.agent_location
        close_to_key_wall = False
        # NORTH
        if (0 <= (r - 1) <= self.map_size - 1) and self.map[r - 1][c] == self.env.items_id[self.wall_name]:
                close_to_key_wall = True
        # SOUTH
        elif (0 <= (r + 1) <= self.map_size - 1) and self.map[r + 1][c] == self.env.items_id[self.wall_name]:
                close_to_key_wall = True
        # WEST
        elif (0 <= (c - 1) <= self.map_size - 1) and self.map[r][c - 1] == self.env.items_id[self.wall_name]:
                close_to_key_wall = True
        # EAST
        elif (0 <= (c + 1) <= self.map_size - 1) and self.map[r][c + 1] == self.env.items_id[self.wall_name]:
                close_to_key_wall = True

        if close_to_key_wall and self.env.inventory_items_quantity[self.key_name] >= 1 and self.env.selected_item == self.key_name :
            result = np.where(self.map == self.items_id['crafting_table'])
            for i in range(len(result[0])):
                r, c = result[0][i], result[1][i]
                # NORTH
                if (0 <= (r - 1) <= self.map_size - 1) and (r - 1, c) != self.agent_location:
                    self.map[r - 1][c] = self.items_id['air']
                # SOUTH
                if (0 <= (r + 1) <= self.map_size - 1) and (r + 1, c) != self.agent_location:
                    self.map[r + 1][c] = self.items_id['air']
                # WEST
                if (0 <= (c - 1) <= self.map_size - 1) and (r, c - 1) != self.agent_location:
                    self.map[r][c - 1] = self.items_id['air']
                # EAST
                if (0 <= (c + 1) <= self.map_size - 1) and (r, c + 1) != self.agent_location:
                    self.map[r][c + 1] = self.items_id['air']
...
def inject_novelty(env, novelty_name, difficulty='hard', novelty_arg1='', novelty_arg2=''):
    ...
    elif novelty_name == 'keywall' :
        env = KeyWall(env, difficulty)
    ...
    elif novelty_name == 'cratescatter':
        #assert novelty_arg1, "For additem novelty, novelty_arg1 (contents of the crate) is needed"

        env = CrateScatter(env, difficulty, novelty_arg1)
